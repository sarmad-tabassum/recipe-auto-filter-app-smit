...
function recipeRender(data, reset = false) {
  ...
  for (let i = currentIndex; i < end && i < data.length; i++) {
    div.innerHTML += `
        <div class="card" onclick="card(${i})">
            <h1>${data[i].name}</h1>
            <img src="${data[i].image}" alt="">
            <p><strong>Difficulty:</strong> ${data[i].difficulty}</p>
            <p><strong>Rating:</strong> ${data[i].rating}</p>
            <p><strong>Cuisine:</strong> ${data[i].cuisine}</p>
            <p><strong>Calories:</strong> ${data[i].caloriesPerServing}</p>
        </div>`;
  }
  currentIndex = end;
...
for (let i = 0; i < recipes.length; i++) {
  if (!difficultyOpt.includes(recipes[i].difficulty)) {
    difficultyOpt.push(recipes[i].difficulty);
  }

  if (!cuisineOpt.includes(recipes[i].cuisine)) {
    cuisineOpt.push(recipes[i].cuisine);
  }
}

for (let i = 0; i < difficultyOpt.length; i++) {
  difficulty.innerHTML += `
  <option value='${difficultyOpt[i]}'>${difficultyOpt[i]}</option>
  `;
}

for (let i = 0; i < cuisineOpt.length; i++) {
  cuisine.innerHTML += `
  <option value='${cuisineOpt[i]}'>${cuisineOpt[i]}</option>
  `;
}
...
function filterData() {
  ...
  for (let i = 0; i < recipes.length; i++) {
    let difficultyMatch =
      difficultyVal == "all" || recipes[i].difficulty == difficultyVal;
    let cuisineMatch = cuisineVal == "all" || recipes[i].cuisine == cuisineVal;

    if (difficultyMatch && cuisineMatch) {
...
searchBtn.addEventListener("click", () => {
  ...
  if (searchVal == "") {
    ...
    for (let i = 0; i < recipes.length; i++) {
      if (recipes[i].name.toLowerCase().includes(searchVal)) {
